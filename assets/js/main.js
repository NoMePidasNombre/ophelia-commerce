...
const totalContainer = document.querySelector(
  ...
  <div class="col-12">
      <div class="shopping-cart-total">
          <p class="agrandar-Total">Total</p>
          <input readonly name="totalPhp" class="shoppingCartTotal cart-total agrandar-Total" value="0$">
          <div class="toast" role="alert" aria-live="assertive" aria-atomic="true"
              data-delay="2000">
          </div>
          <input type="submit" name="boton" class="btn btn-success comprarButton" value="Comprar" onclick="document.comprarprueba.submit(); comprado()">
      </div>
  </div>
...
totalContainer.innerHTML = totalCompra;
 ...
 // comprarButton.addEventListener('click', comprarButtonClicked); //Añade un eventlistener que borra lo que hay dentro de comprar al ser presionado
...
function addItemToShoppingCart(itemTitle, itemPrice, itemImage, itemdetail) {
  ...
  for (let i = 0; i < elementsTitle.length; i++) { //Añade o disminuye la cantidad de 1 producto
    if (elementsTitle[i].value === itemTitle) { //Si en el carrito ya existe el producto, le suma 1 en vez de duplicar
  ...
  <div class="row shoppingCartItem">
        <div class="col-2">
            <div class="shopping-cart-item">
                ...
                <input readonly name="titulosPhp[]" class="shopping-cart-item-title shoppingCartItemTitle cart-title" type="text"
                value="${itemTitle}">
            </div>
        </div>
        <div class="col-4">
        ...
        <div class="col-4">
            <div
                ...
                <input name="cantidadesPhp[]" class="shopping-cart-quantity-input shoppingCartItemQuantity cantidadPhp" type="number"
                    value="1">
    ...
    <?php ?>
    `;    //Creación de la fila HTML de producto
...
function updateShoppingCartTotal() { //Función que comprueba cada vez que se añade o borra un producto, el precio total
  ...
  shoppingCartTotal.value = `${total.toFixed(2)}$`; //Solamente mostrar 2 decimales y agregar nuevamente el valor alfanumérico
}
